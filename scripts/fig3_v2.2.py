"""
Chaplin Figure 3 Recreation — fig3_v2.2.py

Goal:
    Attempt to reproduce the C III 97.7 nm / C IV 155.0 nm line-ratio curve
    (Figure 3) from Chaplin using ColRadPy.

Workflow:
    1. Define plasma temperature and density grids.
    2. Compute ionization balance for carbon.
    3. Extract C III and C IV charge-state fractions.
    4. Independently compute PECs for:
            C III 97.7 nm
            C IV 155.0 nm
    5. Compute:
            ratio = (f_CIII * PEC_97.7) / (f_CIV * PEC_155)
    6. Plot the line ratio and diagnostic component ratios.

Notes:
    Note the scaling factor used for diagnostics - set it to 1.0 for normal scaling.
...
def plot_line_ratio(results):
    ...
    out_file = PROJECT_ROOT / "fig3_v2_2_line_ratio.png"
