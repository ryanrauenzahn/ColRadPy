...
# IMPORTANT:
# This uses the simpler ionization_balance.py file,
# NOT ionization_balance_class.py
...
print("\n--- Building SIMPLE ionization_balance object ---")
...
# Use a long time array, similar in spirit to the official example.
# We only need the final point for the Fig. 4-style equilibrium curve.
times = np.geomspace(1e-8, 200.0, 300)

print("\n--- Solving ionization balance ---")

ib.solve_no_source(n0, times)

pops = np.real(ib.data["pops"])
...
# Safety check: if ColRadPy created extra terminal states, print them.
if offset < n_states:
...
ax.set_title(
    r"Carbon ionization balance from ColRadPy, $n_e = 5\times10^{14}$ cm$^{-3}$",
    fontsize=11,
)
...
out_file = PROJECT_ROOT / "fig4_using_simple_ionization_balance.png"
